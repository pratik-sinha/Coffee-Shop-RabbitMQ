...
import (
	"coffee-shop/config"
	"coffee-shop/internal/barista/server"
...
func main() {
	...
	shutdownTracing, err := tracing.InitTracerProvider(cfg, cfg.Barista_service.ServiceName)
	if err != nil {
		appLogger.Fatalf("Tracing init: %s", err)
	}
	defer shutdownTracing()

	shutdownMetrics, err := metrics.InitMetricsProvider(cfg, cfg.Barista_service.ServiceName)
	if err != nil {
